...
void FRenderer::CreateShader()
{
    ...
    D3D11_INPUT_ELEMENT_DESC StaticMeshLayout[] = {
        ...
        {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0},// 40, D3D11_INPUT_PER_VERTEX_DATA, 0},
        //{"MATERIAL_INDEX", 0, DXGI_FORMAT_R32_UINT, 0, 48, D3D11_INPUT_PER_VERTEX_DATA, 0}
    };

    Graphics->Device->CreateInputLayout(
        StaticMeshLayout, ARRAYSIZE(StaticMeshLayout), VertexShaderCSO->GetBufferPointer(), VertexShaderCSO->GetBufferSize(), &InputLayout
    );

    /** UUID */
    D3DCompileFromFile(L"Shaders/UUIDVertexShader.hlsl", nullptr, nullptr, "mainVS", "vs_5_0", 0, 0, &VertexShaderCSO, nullptr);
    ...
    D3DCompileFromFile(L"Shaders/UUIDPixelShader.hlsl", nullptr, nullptr, "mainPS", "ps_5_0", 0, 0, &PixelShaderCSO, nullptr);
    Graphics->Device->CreatePixelShader(PixelShaderCSO->GetBufferPointer(), PixelShaderCSO->GetBufferSize(), nullptr, &UUIDPixelShader);

    D3D11_INPUT_ELEMENT_DESC UUIDLayout[] = {
        {"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0}
    };

    Graphics->Device->CreateInputLayout(
        UUIDLayout, ARRAYSIZE(UUIDLayout), VertexShaderCSO->GetBufferPointer(), VertexShaderCSO->GetBufferSize(), &UUIDInputLayout
    );
...
void FRenderer::ReleaseShader()
{
    ...
    if (UUIDInputLayout)
    {
        UUIDInputLayout->Release();
        UUIDInputLayout = nullptr;
    }

    if (UUIDPixelShader)
    {
        UUIDPixelShader->Release();
        UUIDPixelShader = nullptr;
    }

    if (UUIDVertexShader)
    {
        UUIDVertexShader->Release();
        UUIDVertexShader = nullptr;
    }
}
...
void FRenderer::PrepareShader() const
{
    ...
    if (ConstantBuffer)
    {
        ...
        Graphics->DeviceContext->PSSetConstantBuffers(1, 1, &SubMeshConstantBuffer);//4, 1, &SubMeshConstantBuffer);
        Graphics->DeviceContext->PSSetConstantBuffers(2, 1, &TextureConstantBufer);//5, 1, &TextureConstantBufer);
    }
}

void FRenderer::PrepareUUIDShader() const
{
    /** UUID */
    Graphics->DeviceContext->VSSetShader(UUIDVertexShader, nullptr, 0);
    Graphics->DeviceContext->PSSetShader(UUIDPixelShader, nullptr, 0);
    Graphics->DeviceContext->IASetInputLayout(UUIDInputLayout);

    if (UUIDConstantBuffer)
    {
        Graphics->DeviceContext->VSSetConstantBuffers(0, 1, &UUIDConstantBuffer);
        Graphics->DeviceContext->PSSetConstantBuffers(0, 1, &UUIDConstantBuffer);
    }
}
...
void FRenderer::CreateConstantBuffers()
{
    ...
    Graphics->Device->CreateBuffer(&constantbufferdesc, nullptr, &UUIDConstantBuffer);
...
void FRenderer::ReleaseConstantBuffers()
{
    ...
    ReleaseBuffer(UUIDConstantBuffer);
    ...
    ReleaseBuffer(TextureConstantBufer);
}
...
void FRenderer::UpdateConstant(const FMatrix& MVP, bool IsSelected) const//, const FMatrix& NormalMatrix, FVector4 UUIDColor, bool IsSelected) const
{
    if (ConstantBuffer)
    {
...
void FRenderer::UpdateUUIDConstant(const FMatrix& MVP, FVector4 UUIDColor) const
{
    if (UUIDConstantBuffer)
    {
        D3D11_MAPPED_SUBRESOURCE UUIDConstantBufferMSR;

        Graphics->DeviceContext->Map(UUIDConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &UUIDConstantBufferMSR); // update uuid buffer every frame
        {
            FUUIDConstants* constants = static_cast<FUUIDConstants*>(UUIDConstantBufferMSR.pData);
            constants->MVP = MVP;
            constants->UUIDColor = UUIDColor;
        }
        Graphics->DeviceContext->Unmap(UUIDConstantBuffer, 0);
    }
}
...
ID3D11Buffer* FRenderer::CreateStaticVerticesBuffer() const
{
    ...
    Graphics->Device->CreateBuffer(&vbDesc, &vbInitData, &pVertexBuffer);
...
) const
{
    ...
    //@TODO: 순서에 민감함 (RenderStaticMesh 앞에 호출되어야만 함). 이를 해결할 수 있는 더 나은 방안 고민.
    Graphics->DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
}
...
{
    //@TODO: EngineLoop::Render의 PrepareRender() 앞으로 이동
...
void FRenderer::Render(UWorld* World, std::shared_ptr<FEditorViewportClient> ActiveViewport)
{
    ...
    Graphics->Prepare();
    //@Note: 반드시 RenderStaticMeshes 앞에서 호출되는 것이 보장되어야 함 (FRenderer::RenderBatch 참고)
    UPrimitiveBatch::GetInstance().RenderBatch(ActiveViewport->GetViewMatrix(), ActiveViewport->GetProjectionMatrix());

    //if (ActiveViewport->GetShowFlag() & static_cast<uint64>(EEngineShowFlags::SF_Primitives))
    //{
        //** Culling StaticMeshObjs */
        FrustumCulling(ActiveViewport);
        if (!GEngineLoop.GetIsMoving())
        {
            // Use Previous Frame's Rendered StaticMeshObjs Info
            StaticMeshObjs = PostFrameRenderedStaticMeshObjs;
        }

        RenderStaticMeshes(World, ActiveViewport);

        Graphics->PrepareZPrepass();
        RenderUUIDs(World, ActiveViewport);
    //}
    Graphics->SwitchRTV(Graphics->FrameBufferRTV);
...
void FRenderer::RenderStaticMeshes(const UWorld* World, const std::shared_ptr<FEditorViewportClient>& ActiveViewport)
{
    ...
    for (UStaticMeshComponent* StaticMeshComp : StaticMeshObjs)
    {
        ...
        {
            UpdateConstant(MVP, true);//, NormalMatrix, UUIDColor, true);
        }
        else
        {
            UpdateConstant(MVP, false);//, NormalMatrix, UUIDColor, false);
        }
        UpdateTextureConstant(0, 0);

        //TODO: AABB 박스 그리지 않는 중
...
void FRenderer::RenderUUIDs(UWorld* World, std::shared_ptr<FEditorViewportClient> ActiveViewport)
{
    PrepareUUIDShader();

    FScopeCycleCounter meshRenderTimer{ TEXT("UUIDs") };
    ...
    for (UStaticMeshComponent* StaticMeshComp : StaticMeshObjs)
    {
        ...
        UpdateUUIDConstant(MVP, UUIDColor);
...
void FRenderer::UpdatePostFrameRenderedStaticMeshObjs()
{
    ...
    for (uint32 UUID : RenderedUUIDs)
    {
        for (UStaticMeshComponent* StaticMesh : StaticMeshObjs)
        {
            if (!StaticMesh)
                continue;
            if (StaticMesh->GetUUID() == UUID)
            {
                PostFrameRenderedStaticMeshObjs.Emplace(StaticMesh);
                break;
            }
        }
    }
